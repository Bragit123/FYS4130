...
def run_MC(d, L, T, q=3, n_clusters=10, n_steps_eq=100_000, n_steps=100_000, n_runs=5):
    N = L if d == 1 else L * L
    ...
    for run in range(n_runs):
        S = np.random.randint(0, q, size=N)  # Start from random config
        ...
        m2 = np.mean(np.abs(mt)**2)
        m4 = np.mean(np.abs(mt)**4)
        gamma = m4 / (m2**2 + 1e-12)
        gamma_accum += gamma

    gamma_avg = gamma_accum / n_runs
    return gamma_avg
...
for L in Ls:
    ...
    for i, T in enumerate(T_vals):
        ...
        gamma_vals[i] = run_MC(d, L, T, n_runs=5)
...
plt.legend()
plt.grid(True)
plt.savefig("gamma_crossing2.pdf")
plt.show()
